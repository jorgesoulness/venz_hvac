...
function seopress_pro_tools_before( $current_tab, $docs ) {
	if ( version_compare( SEOPRESS_PRO_VERSION, '6.1', '>=' ) || ( defined( 'SCRIPT_DEBUG' ) && SCRIPT_DEBUG === true ) ) { ?>
		...
		<?php
		if ( 'tab_seopress_tool_data' === $current_tab ) {
			echo 'active'; }
		?>
...
<div class="seopress-tab 
	<?php
	if ( 'tab_seopress_tool_redirects' === $current_tab ) {
			echo 'active';
	}
	?>
	" id="tab_seopress_tool_redirects">
			<?php
			if ( '1' == seopress_get_toggle_option( '404' ) && function_exists( 'seopress_get_redirection_pro_html' ) ) {
		...
		<div class="seopress-tab 
		<?php
		if ( 'tab_seopress_tool_video' === $current_tab ) {
			echo 'active';
		}
		?>
